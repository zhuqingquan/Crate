...
//全局变量 dx 表示两个像素之间归一化间隔，如有100个像素，则dx = 1/100
float dx;
...
float4 PS(VertexOut pin, uniform int gLightCount, uniform bool gUseTexure) : SV_Target
{
	...
	{
		// Sample texture.
		float y = yTexture.Sample( samLinear, pin.Tex ).x;//yTexture.Sample( samLinear, pin.Tex ).x;
		float u = uTexture.Sample( samLinear, pin.Tex ).x;
		float v = uTexture.Sample( samLinear, pin.Tex ).y;
		//float pos = uTexture.Sample( samLinear, pin.Tex ).x;
		//if(pos>0.5)
		//{
		//	v = oneSample.y;
